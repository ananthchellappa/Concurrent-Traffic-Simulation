...
void Graphics::loadBackgroundImg()
{
    ...
    cv::resizeWindow(_windowName, 1000,800);
...
void Graphics::drawTrafficObjects()
{
    ...
    for (auto it : _trafficObjects)
    {
        ...
        if (it->getType() == ObjectType::objectIntersection)
        {
            ...
            // I changed to blue just to make it easier to see
            cv::Scalar trafficLightColor = intersection->trafficLightIsGreen() == true ? cv::Scalar(0, 255, 055) : cv::Scalar(0, 0, 255);
