...
void MessageQueue<T>::send(T &&msg)
{
    ...
    _cond.notify_one();   // "notify client after pushing"
}
...
void TrafficLight::waitForGreen()
{
    ...
    // do we need to add delay here? why not specify clearly? 
    while( true ){
        if( green == _lightPhases.receive() )
            return;     // this function will block the caller till we return
...
void TrafficLight::cycleThroughPhases()
{
    ...
    while( true ){
        ...
        if ( red == _currentPhase )
            ...
            _currentPhase = red;
        std::this_thread::sleep_for(std::chrono::milliseconds(1) );
        // if you have not done anything with MessageQueue yet, then how do you know what method is available for this?
        // "sends an update method to the m q" -- asked on forum

        _lightPhases.send(std::move(_currentPhase)); // help from Nayir
    }
}
